...
> - **sensor_snapshot**：基于 `(device_id, timestamp)` 去重。若 `timestamp` 精度不足（秒级），结合 IoTDA 消息中的 `event_time`（毫秒级）生成唯一键，使用 `INSERT ... ON CONFLICT (device_id, timestamp) DO NOTHING`（KingbaseES 兼容 PostgreSQL 的 ON CONFLICT 语法）
> - **disease_records**：基于 `(device_id, timestamp)` 去重，同上策略
...
4. 更新设备在线状态（`devices` 表的 `last_seen` 和 `online` 字段；同时更新进程内 dict）
5. 返回 200 OK
...
**处理逻辑**：
1. 写入 `disease_records` 表（使用 `INSERT ... ON CONFLICT (device_id, timestamp) DO NOTHING` 保证幂等）；若数据库写入失败，返回 HTTP 500（触发 IoTDA 侧重试机制），并记录错误日志
...
| 15 | GET | `/api/v1/export/sensor` | 导出传感器历史数据（CSV/Excel） | 上位机 |
| 16 | GET | `/api/v1/health` | 健康检查（数据库连通性、服务运行时间） | Docker healthcheck / 运维 |
...
HTTP 状态码：healthy/degraded 时返回 200，unhealthy 时返回 503。
