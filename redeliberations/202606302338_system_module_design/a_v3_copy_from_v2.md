...
| v4 | 2026-06-30 | 第4轮修订，修复27项强制项（严重1/高4/中9/低13）+ 跨文档协同标注 |
| v5 | 2026-07-01 | 第5轮修订，修复6项问题（中4/低2）：索引类型修正、API字段一致性、联动分析字段补全、日聚合查询API、健康检查状态码、command_id时序说明 |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **C-03**（严重）：`system_architecture_relationship.md` 文件不存在，§1.1/§2.5/§5.1/§6.6 多处引用 | §1.1 术语等价说明改写为自包含描述；§2.5 表1注释移除外部引用改为独立表名选择说明；§5.1 docs/ 目录移除该文件条目；§6.6 移除外部文档引用，保留独立表名理由 |
| **H-01**（高）：规格说明书（华为云存储）与模块设计（VPS KingbaseES）数据存储矛盾未仲裁 | §6.1 增加显式分歧对比表（6 维度：成本/国产化/数据主权/开发便利性/集成延迟/规格一致性）与仲裁结论；标注 `system_specification.md` §2.3 需协同修订 |
| **H-02**（高）：上位机"历史数据导出"职责有定义但无对应 API | 新增 §4.9 `GET /api/v1/export/sensor` 端点（参数 `device_id`/`start`/`end`/`format`，返回 CSV/XLSX 文件流）；§4.8 接口清单新增第15项；§2.7 上位机补充导出对接说明 |
| **H-04**（高）：`/sensor/latest` 多设备场景响应单对象与接口语义冲突 | §4.3.1 响应格式改为 `records` 数组（始终返回数组，即使只有一条记录），统一单/多设备场景解析逻辑 |
| **H-06**（高）：光照报警阈值 `light < 100`（量程 0-100）导致持续误报 | §2.1 alarm_flag 表低光照阈值改为 `light < 20`（作物光合作用最低需求）；§2.4 告警阈值表同步修正；标注 `DATA_INVENTORY.md` §2.3 需同步修订 |
| **M-01**（中）：缺少独立设备信息表，在线状态仅存内存缓存 | §2.5 新增 `devices` 表（字段：`device_id`/`device_name`/`mac_addr`/`registered_at`/`last_seen`/`online`）；§4.3.3 设备列表接口增加数据来源说明；§2.4 在线状态存储改为"进程内 dict + devices 表持久化" |
| **M-03**（中）：IoTDA Webhook 缺少幂等性保障 | §4.2 三 Webhook 路由规则后增加幂等性说明（基于 `(device_id, timestamp)` 或 `command_id` + `ON CONFLICT DO NOTHING`）；§4.2.1–§4.2.3 处理逻辑均增加幂等性步骤 |
| **M-05**（中）：缺少 API 健康检查端点 | 新增 §4.10 `GET /api/v1/health` 端点（返回 `status`/`uptime_seconds`/`db_connected`/`version`）；§4.8 接口清单新增第16项；§5.2 api 服务增加 healthcheck 配置 |
| **M-06**（中）：MQTT 仅使用明文端口 1883，未涉及 TLS | §2.3 MQTT 连接参数表改为开发/生产双环境（增加 TLS 8883 端口列）；增加 TLS 证书配置说明；标注 `DATA_INVENTORY.md` §3.4 需同步更新 |
| **M-07**（中）：联动分析结果写入时序/存储位置不明确 | §2.4 联动监测逻辑增加步骤3（持久化写入 `disease_records` 表的 `linkage_risk_level`/`linkage_detail`）；§2.5 `disease_records` DDL 新增联动字段；§3.2 流 B 序列图增加联动分析 DB 写入步骤 |
| **M-08**（中）：`/sensor/history` 响应 records 元素缺少 `device_id` | §4.3.2 响应 JSON 中 records 元素增加 `device_id` 字段 |
| **M-09**（中）：决策规则矩阵未覆盖"环境条件不满足"行为 | §2.4 决策规则矩阵增加"条件不满足时行为"列（`action=manual_inspect`，防治建议仍生成并通过 `/api/v1/advisory` 返回） |
| **S-01**（中）：设备端 `service_id` 携带方式未明确 | §2.1 增加"数据上报策略"小节（`farmeye_env`/`farmeye_ai` 两个 service_id 及对应 payload 模板说明）；§3.1 交互矩阵更新为包含 service_id 区分 |
| **S-02**（中）：设备在线判定 Redis 未在部署方案体现；进程内 dict 未分析多 worker 不一致 | §2.4 在线状态存储明确为"进程内 dict + 单 worker + devices 表持久化"；增加多 worker 场景升级说明（引入 Redis）；初版不引入 Redis |
| **L-01**（低）：AI 推理输出 JSON 缺失 `timestamp` 字段 | §2.2 AI 输出 JSON 增加 `timestamp` 字段（ISO 8601 格式） |
| **L-02**（低）：大屏第11项"事件驱动"实际依赖 HTTP 轮询 | §2.7 上位机大屏第11项刷新方式从"事件驱动"改为"按轮询周期（10s），有更新时高亮显示" |
| **L-03**（低）：`alarm_flag` 中 `soil_k` 无 bit 位但未说明理由 | §2.1 alarm_flag 表后增加 bit 预留说明（钾素缺乏由 AI 图像识别检测更合适 + 推导值不参与板端报警） |
| **L-04**（低）：语音播报内容未在模块设计中定义 | §2.1 增加语音播报内容定义表（13 条预置语音文本，覆盖全部 8 种报警 + 4 种 AI 识别 + 重度告警） |
| **L-05**（低）：`control_logs` 查询接口响应缺少 `command_id` | §2.5 `control_logs` DDL 增加 `command_id` 字段（VARCHAR(64)）+ UNIQUE 索引；§4.5.2 响应 records 增加 `command_id`；§4.2.3 移除过时的 DDL 变更提醒 |
| **L-06**（低）：KingbaseES healthcheck `ksql` 命令可能不兼容 | §5.2 db 健康检查命令改为三级回退：`pg_isready` → `ksql` → TCP 端口检测（`nc -z`），增加 `timeout`/`start_period` 参数 |
| **L-07**（低）：分页 `page_size` 上限 100 超出时行为未定义 | §4.1 分页约定增加超出上限行为说明（自动截断为 100，返回 HTTP 200 并回显实际值） |
| **L-09**（低）：`sensor_daily_aggregation` 表结构未定义 | §2.5 新增表5 `sensor_daily_aggregation` 完整 DDL（日均值/最大/最小 + `record_count`）；数据保留策略更新引用 |
| **L-10**（低）：热力图在单设备场景缺少空间坐标数据 | §4.4.3 热力图说明细化（单设备基于时间轴热力条，多设备基于设备坐标聚合），增加位置坐标扩展预留说明 |
| **L-12**（低）：`/api/v1/advisory` 缺少时间窗口参数 | §4.6.1 增加可选查询参数 `start`/`end`/`window_minutes`，明确参数优先级 |
| **L-13**（低）：鸿蒙 App 缺少测试目录，上位机缺少测试目录 | §5.1 `harmony-app/` 增加 `ohosTest/` 模块；`host-computer/` 增加 `tests/` 目录 |
| **L-14**（低）：HC-SR04 超声波传感器在规格说明书中未被提及 | §6.5 增加跨文档协同说明，建议 `system_specification.md` §3 硬件架构表补充 HC-SR04 |
| **S-03**（低）：API 服务 docker-compose 缺少自身 healthcheck | §5.2 api 服务增加 `healthcheck` 配置（`curl -f /api/v1/health`）；`depends_on` 改为 `service_healthy` |
| **S-04**（低）：`/api/v1/command/logs` 缺少按 `source`/时间范围筛选参数 | §4.5.2 增加可选查询参数 `source`/`start`/`end` |
| **认证方案补充** | §4.1 认证方式改为 API Key 双模式（开发期可选无认证，生产须启用 `X-API-Key` 头校验）；§5.2 docker-compose 增加 `API_KEYS` 环境变量 |

**跨文档协同修订清单**（本设计文档已标注，需协同修改以下文件）：

| 涉及文档 | 修订项 | 本设计对应位置 |
|---------|--------|--------------|
| `docs/system_specification.md` §2.3 | H-01：数据存储位置矛盾，建议标注 VPS + KingbaseES 实践 | §6.1 |
| `docs/system_specification.md` §3 | L-14：硬件架构表增加 HC-SR04 超声波测距模块 | §6.5 |
| `docs/DATA_INVENTORY.md` §2.3 | H-06：光照报警阈值从 `< 100` 同步修正为 `< 20` | §2.1, §2.4 |
| `docs/DATA_INVENTORY.md` §3.4 | M-06：MQTT 连接参数表增加 TLS 端口 8883 配置 | §2.3 |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **N-01**（中）：`sensor_snapshot` / `disease_records` DDL 索引为普通 `CREATE INDEX`，无法满足 `ON CONFLICT` 幂等性要求 | §2.5 表1 `idx_sensor_device_time` 改为 `CREATE UNIQUE INDEX`；§2.5 表2 `idx_disease_device_time` 改为 `CREATE UNIQUE INDEX ON disease_records (device_id, timestamp, disease_type)`（增加 `disease_type` 列以支持多病虫害类型同秒上报）；§4.2 幂等性说明增加 `event_time` 毫秒精度要求（优先使用 IoTDA 推送的 `event_time` 而非设备端秒级时间戳）；§4.2.2 处理逻辑同步更新为 `ON CONFLICT (device_id, timestamp, disease_type)` |
| **N-02**（中）：`devices` DDL 缺 `ip_addr`；`/sensor/history` 响应缺 `ip_addr` / `mac_addr` | §2.5 表4 `devices` DDL 增加 `ip_addr VARCHAR(16)` 字段；§4.2.1 处理逻辑步骤4 增加同步更新 `devices` 表 `ip_addr` 说明；§4.3.2 `/sensor/history` 响应 records 元素增加 `ip_addr` 和 `mac_addr` 字段，与 `/sensor/latest` 格式统一 |
| **O-01**（中）：`disease_records` API 响应缺少联动分析字段，历史追溯意图不可实现 | §4.4.1 病虫害记录列表响应 records 元素增加 `linkage_risk_level` 和 `linkage_detail` 两个字段 |
| **O-02**（中）：`sensor_daily_aggregation` 日聚合表缺少查询 API，"聚合-保留-查询"链路在查询端断开 | 新增 §4.3.3 `GET /api/v1/sensor/daily` 端点（参数 `device_id`/`start`/`end`/`page`/`page_size`，返回日均值/最大/最小值 + `record_count`）；原 §4.3.3 设备列表接口重编号为 §4.3.4；§4.8 接口清单新增第6项，后续条目重编号为 7-17 |
| **O-03**（低）：健康检查 `degraded` 状态返回 200，Docker healthcheck 无法感知部分降级 | §4.10.1 健康判定逻辑：`degraded` HTTP 状态码从 200 改为 503；`healthy` 保持 200，`unhealthy` 保持 503；§5.2 api 服务 healthcheck 命令改为 `grep -q '"status":"healthy"'` 解析 JSON 响应体，避免 `curl -f` 因 503 状态码误判 healthy |
| **O-04**（低）：`control_logs.command_id` 为 NULL 的时序场景未定义，窗口期重复插入风险 | §4.5.1 新增"处理逻辑"步骤（含"预先生成 `command_id`"：API 侧在 INSERT 时即写入而非等待 IoTDA 回填）和"`command_id` 为 NULL 的场景说明"（场景一：自动触发命令已有 command_id，插入时即确定不为 NULL；场景二：历史遗留记录无 command_id）；保留可为 NULL 约束以兼容历史数据和纯本地控制动作 |
