# 诊断报告：FarmEye Guard 测试失败根因分析 (v3)
...
3. **命令下发的在线检查**：`server\app\services\command_service.py:38-40` —— `create_command()` 检查 `not device.online`，返回 `{"status": "offline", "code": 1003}`。
...
**同时注意**：在修改字段定义之前，还需在三个模型文件的 `from sqlalchemy import ...` 行追加 `text`。若不添加此导入，修改后的代码将抛出 `NameError: name 'text' is not defined`。具体改动：

| 文件 | 当前 import 行 | 修改后 |
|------|---------------|--------|
| `sensor.py:8` | `from sqlalchemy import Column, BigInteger, String, Integer, SmallInteger, Numeric, DateTime, Date, UniqueConstraint` | `from sqlalchemy import Column, BigInteger, String, Integer, SmallInteger, Numeric, DateTime, Date, UniqueConstraint, text` |
| `disease.py:6` | `from sqlalchemy import Column, BigInteger, String, SmallInteger, Numeric, DateTime` | `from sqlalchemy import Column, BigInteger, String, SmallInteger, Numeric, DateTime, text` |
| `control.py:8` | `from sqlalchemy import Column, BigInteger, String, Integer, SmallInteger, Boolean, DateTime` | `from sqlalchemy import Column, BigInteger, String, Integer, SmallInteger, Boolean, DateTime, text` |
...
为便于对照故障路径与推荐方案的关系，以下给出编号映射：

- **方案A（注册时直接设 True）** → 仅覆盖**路径B**（仅修 `if` 分支，新建设备时设 `online=True`；不覆盖 `else` 分支，seed data 预置离线的场景无法修复）
- **方案B（上报数据时更新 `online=True`）** → 同时覆盖**路径A**和**路径B**（`if` 分支和 `else` 分支均设 `online=True`，两条故障路径均被修复）
- **方案C（保持离线 + 增加上线步骤）** → 不直接映射任一故障路径（需新增 API 端点和测试脚本修改，工程成本高）
...
**先决条件说明**：`create_snapshot()` 当前第33行未捕获 `ensure_device_exists()` 的返回值，代码为 `ensure_device_exists(db, device_id, properties.get("mac_addr"))`。修复者若选择此路径需先将第33行改为 `device = ensure_device_exists(db, device_id, properties.get("mac_addr"))`，再设置 `device.online = True`。

#### 关于 `ensure_device_exists()` docstring 的更新

`ensure_device_exists()` 的 docstring 当前记载 `online=False`（新建设备时），修复方案改为 `online=True` 后，docstring 需同步更新。具体修改位置：`sensor_service.py:68`，将 `online=False` 改为 `online=True`。

#### 验收建议

修复后验证：
1. **路径A（E2E）**：重新运行 `python tests/integration_run.py`，确认步骤6返回 `{"status": "sent"}` 而非 `{"status": "offline", "code": 1003}`
2. **路径B（自动注册）**：待问题1修复后，运行集成测试确认 `test_online_default_false` 测试用例按预期通过（该测试验证 Python 模型的 `default=False`，与 `ensure_device_exists` 业务逻辑独立。修复后 `default=False` 保持不变，该测试仍应正常通过）
3. **新增集成测试**：建议新增一个集成测试用例，在 `ensure_device_exists` 调用链路的上下文中验证设备 `online=True`，以覆盖设备上报数据后在线状态应为 `True` 的集成验证场景
