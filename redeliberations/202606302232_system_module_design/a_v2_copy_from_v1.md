...
| 2 | rust (锈病) | 2 | `humidity > 85%` 或 `temperature 15-25℃` | 无自动 | 48h 内喷施三唑酮 |
| 3 | rust (锈病) | 3 | 任意 | `spray ON` | 立即喷施杀菌剂，隔离病区 |
| 4 | powdery_mildew (白粉病) | 1 | — | 无自动 | 加强通风，降低湿度 |
| 5 | powdery_mildew (白粉病) | 2 | `humidity 50-80%` | 无自动 | 喷施嘧菌酯 |
| 6 | powdery_mildew (白粉病) | 3 | 任意 | `spray ON` | 立即喷施杀菌剂 |
| 7 | anthracnose (茶炭疽病) | 1 | — | 无自动 | 检查茶园湿度 |
| 8 | anthracnose (茶炭疽病) | 2 | `humidity > 80%` 或 `temperature 20-30℃` | 无自动 | 喷施苯醚甲环唑 |
| 9 | anthracnose (茶炭疽病) | 3 | 任意 | `spray ON` | 立即喷施杀菌剂 |
| 10 | leafhopper (茶小绿叶蝉) | 1 | — | 无自动 | 监控虫口密度 |
| 11 | leafhopper (茶小绿叶蝉) | 2 | `temperature 20-30℃` | 无自动 | 喷施吡虫啉 |
...
| 表 | 策略 | 说明 |
|----|------|------|
| `sensor_snapshot` | 保留最近 30 天明细；30 天前数据按天聚合（取均值/最大/最小）后仅保留聚合记录 | 定时任务每日凌晨执行 |
...
在华为云 IoTDA 控制台的"数据流转规则"中配置两条规则，将传感器数据和 AI 识别结果分别路由至不同端点：
...
配置要点：
- 在 IoTDA 规则引擎中通过 SQL 条件 `WHERE service_id = 'farmeye_env'` 和 `WHERE service_id = 'farmeye_ai'` 区分两条规则
- 两条规则可指向同一 VPS 的不同路径，或不同端口的不同路径
...
| 8 | GET | `/api/v1/disease/heatmap` | 病虫害热力图数据 | 鸿蒙 App / 上位机 |
| 9 | POST | `/api/v1/command` | 下发设备控制命令 | 鸿蒙 App / 上位机 |
| 10 | GET | `/api/v1/command/logs` | 查询控制日志 | 鸿蒙 App / 上位机 |
| 11 | GET | `/api/v1/advisory` | 获取防治建议 | 鸿蒙 App / 上位机 |
| 12 | POST | `/api/v1/image/upload` | 上传病虫害图片 | 鸿蒙 App / 上位机 |
| 13 | GET | `/api/v1/image/{image_id}` | 获取图片 | 鸿蒙 App / 上位机 |
