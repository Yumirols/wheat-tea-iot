...
| # | 审查意见 | 处理方式 |
|---|---------|---------|
| C1 | 病虫害严重度等级数量与规格书矛盾（4级→3级） | 修改：删除"Normal"等级，改为 Mild/Moderate/Severe 三级，severity_code 改为 1-3；同步修改 §2.2 分级表、§4.3.2 history 响应（移除 severity 引用）、§4.4.1 查询参数和响应、§4.4.2 stats 响应、§4.6.1 advisory 响应 |
| C2 | alarm_flag 位掩码完全未定义 | 修改：在 §2.1 上报数据字段说明后新增 alarm_flag 位掩码定义表（引用 DATA_INVENTORY.md §2.12，含 8 个位：0x01–0x80） |
| C3 | 告警阈值完全未定义 | 修改：在 §2.4 决策引擎职责后新增"告警阈值定义"表，列出全部 8 项阈值（温度 >38.0/<-10.0、湿度 >90%/<15%、光照<100、CO2>2000、soil_n<30、soil_p<10） |
| C4 | 土壤 NPK 推导公式缺失 | 修改：在 §2.1 上报数据字段说明后写入完整推导公式（soil_n/p/k），并在 §6.5 中同步写入并注明与规格书的矛盾 |
| H1 | 病虫害记录 API 响应中缺少 image_path | 修改：在 §4.4.1 疾病记录列表响应的 record 对象中增加 `image_path` 字段 |
| H2 | 缺少图片上传/获取 API 端点 | 修改：新增 §4.7 图片接口，包含 `POST /api/v1/image/upload` 和 `GET /api/v1/image/{image_id}` 两个端点；更新 §4.8 接口清单汇总表和 §5.1 工程结构（新增 image.py） |
| H3 | 缺少热力图功能 | 修改：新增 §4.4.3 `GET /api/v1/disease/heatmap` 端点；更新 §4.8 接口清单汇总表 |
| H4 | 缺少环境数据与病虫害发生的联动监测分析 | 修改：在 §2.4 新增"环境-病虫害联动监测逻辑"说明（3 步流程）；在 §4.6.1 advisory 响应中增加 `env_disease_linkage` 字段；在流 B 中增加联动分析步骤 |
| H5 | 设备在线/离线判定机制未定义 | 修改：在 §2.4 新增"设备在线/离线判定机制"说明（30s 无上报=离线，定时任务+内存缓存） |
| H6 | 鸿蒙端消息推送机制空白 | 修改：在 §2.6 新增"消息推送技术方案"表（HTTP 轮询→WebSocket 渐进升级，理由说明） |
| M1 | IoTDA 双 Webhook 规则配置方案缺失 | 修改：在 §4.2 开头新增"双 Webhook 路由规则配置方案"表和配置要点说明 |
| M2 | sensor_snapshot 表名与架构文档 sensor_data 差异未说明 | 修改：在 §2.5 表1 SQL 前增加表名说明引用块；在 §6.6 新增专节解释命名选择理由 |
| M3 | 缺少完整的决策规则矩阵 | 修改：在 §2.4 新增"决策规则矩阵"表（12 条规则，覆盖病虫害类型×严重度×关键环境条件组合） |
| M4 | 缺少数据保留/清理策略 | 修改：在 §2.5 表定义后新增"数据保留与清理策略"表（sensor_snapshot 30天、disease_records 永久、control_logs 90天）；在 §5.2 docker-compose 中增加 DATA_RETENTION 环境变量 |
| M5 | GPIO 表中 OLED 行缺少 I2C 地址 | 修改：在 §2.1 GPIO 表中 GPIO_15 行追加 I2C 地址 `0x3C` |
| M6 | distance=-1 含义未说明 | 修改：在 §2.1 上报数据字段表中 distance 行说明追加"`-1` 表示超时/无目标，服务端应忽略或标记为无效" |
| L1 | 架构命名缺少与参考文档的等价关系 | 修改：在 §1.1 系统定位后新增"架构术语等价说明"引用块，标注与 system_architecture_relationship.md 的对应关系 |
| L2 | 缺少命令应答 payload 格式 | 修改：在 §2.3 MQTT Topic 约定表后新增"命令应答 payload 格式"说明（引用 DATA_INVENTORY.md §3.3 的 result_code/response_name/paras 结构） |
| L3 | 缺少与规格书关于 NPK 传感器的矛盾说明 | 修改：在 §6.5 末尾追加矛盾说明引用块，标注规格书列出 NPK 传感器但实际硬件未接入，当前采用推导方案，后续预留 RS485 扩展 |
| L4 | 防治建议 1h 窗口缺少选择依据 | 修改：在 §4.6.1 说明中追加"1 小时窗口选择依据"和配置项 `ADVISORY_WINDOW_MINUTES`（默认 60） |
| L5 | 缺少 firmware/tests/ 目录或嵌入式测试策略 | 修改：在 §5.1 工程结构中新增 `firmware/tests/` 目录（含 test_sensor.c、test_mqtt.c、test_alarm.c）；在 §6.7 新增嵌入式端测试策略说明 |
| L6 | 金仓数据库镜像 tag 未固定 | 修改：在 §5.2 docker-compose 中将 `kingbase/kb_v8:latest` 改为 `kingbase/kb_v8:V008R006C008B0020` |
| L7 | 缺少预留错误码（1004/1005/3001） | 修改：在 §4.1 通用约定错误码表中新增 1004（认证失败）、1005（请求频率限制）、3001（IoTDA 调用失败） |
| L8 | sensor/latest 与 sensor/history 返回字段不统一 | 修改：统一 §4.3.1 和 §4.3.2 的历史记录的 records 字段集，包含 temperature/humidity/light/co2/soil_n/soil_p/soil_k/distance/rssi/alarm_flag；latest 额外含 device_id/timestamp/ip_addr/mac_addr |
| L9 | 缺少日志配置模块占位 | 修改：在 §5.1 工程结构 `server/app/core/` 下新增 `logging_config.py` 日志配置模块占位 |
| L10 | 缺少实时数据获取方式的选择说明 | 修改：在 §2.4 末尾新增"实时数据获取方式选择"说明（推荐 HTTP 轮询，理由说明） |
| L11 | 上位机大屏展示环境参数清单未明确 | 修改：在 §2.7 新增"大屏展示环境参数清单"表（12 项参数及显示形式） |
| L12 | 缺少 API 版本管理策略说明 | 修改：在 §4.1 通用约定中新增"API 版本管理策略"说明（路径前缀版本化、向后兼容规则） |
| L13 | 图表库选择未明确 | 修改：在 §2.7 技术栈行明确选择 PyQtGraph，新增"图表库选择理由"说明 |
| L14 | sensor/history 查询响应缺少 alarm_flag | 修改：已在 §4.3.2 历史记录响应中增加 `alarm_flag` 字段（与 L8 合并处理） |

## 修订说明（v3）

| # | 审查意见 | 处理方式 |
|---|---------|---------|
| N1 | 图片存储持久化机制未定义 | 修改：在 §5.2 docker-compose.yml 中为 api 服务增加图片存储 bind mount 挂载（`- ./images:/app/images`）和环境变量 `IMAGE_STORAGE_PATH`；在 §4.7 新增 §4.7.3 图片存储策略，明确宿主机/容器路径映射关系、目录组织和持久化保证 |
| N2 | IoTDA 命令应答 Webhook 回传路径不完整 | 修改（方案 A）：在 §4.2 新增第三条 Webhook 规则 `rule_cmd_response_forward`（topic 匹配 `.../sys/commands/response/...`），配置说明更新为"三 Webhook 路由规则配置方案"；新增 §4.2.3 命令应答接收端点 `POST /api/v1/iotda/cmd/response`；在 §3.2 流 B 时序图中明确标注命令应答通过新规则回传至 `/iotda/cmd/response` 端点；更新 §4.8 接口清单汇总表（新增序号 3）；标注 `control_logs` 表需在详细设计阶段补充 `command_id` 字段 |
| N3 | 决策规则矩阵中环境条件边界值未明确开闭区间 | 修改：将 §2.4 决策规则矩阵中 4 处连字符范围表示统一为明确不等式——`temperature 15-25℃` → `15℃ ≤ temperature ≤ 25℃`、`humidity 50-80%` → `50% ≤ humidity ≤ 80%`、`temperature 20-30℃` → `20℃ ≤ temperature ≤ 30℃`（边界值取闭区间，因适温/适湿范围通常含端点值） |
| N4 | 数据聚合后存储的表结构未定义 | 修改：在 §2.5 数据保留与清理策略表中 `sensor_snapshot` 行说明字段补充"聚合表结构（如 `sensor_daily_aggregation`）将在详细设计阶段定义" |
| N5 | Python API 端 Webhook 异常处理策略未说明 | 修改：在 §4.2.1 和 §4.2.2 的处理逻辑步骤中增加错误处理说明——数据库写入失败时返回 HTTP 500（触发 IoTDA 侧重试机制），并记录错误日志 |
| — | 质询报告额外提示：时间戳格式不一致 | 酌情修改：在 §4.1 通用约定时间戳格式行说明中补充"IoTDA Webhook 推送的 `event_time` 使用 UTC 时区后缀 `Z`，接收端需兼容处理" |
