...
| 审查意见 | 处理方式 |
|---------|---------|
| start.sh 及部署步骤未使用生产覆写文件，与 §3.2.3 的生产覆写要求不一致 | **修改**。更新了 `deploy/scripts/start.sh`（§3.6.1）、`deploy/scripts/stop.sh`（§3.6.2）、`deploy/scripts/restart.sh`（§3.6.3）以及 VPS 部署流程（§3.2.2）和部署循环（§5.1.3）中的所有 `docker compose` 命令，统一使用 `docker compose -f docker-compose.yml -f docker-compose.prod.yml --compatibility up -d --build` 格式，确保 Nginx 服务和生产覆写配置始终生效。 |
| pytest 钩子函数定义在 `test_e2e.py` 测试文件中，pytest 不会从 `test_*.py` 发现引导阶段钩子 | **修改**。将 `pytest_addoption`、`pytest_configure`、`pytest_collection_modifyitems` 三个钩子函数从 `test_e2e.py` 移至 `tests/conftest.py`（§4.1.3），并补充了详细的文档注释，说明钩子函数必须定义在 conftest.py 中的理由。同时，为每个 marker 注册了描述信息，确保 `pytest --markers` 可正确显示。 |

## 修订说明（v4）

| 审查意见 | 处理方式 |
|---------|---------|
| §1.4 的 Dockerfile prod stage 以 `CMD` 结尾，未包含 `COPY entrypoint.sh` 和 `ENTRYPOINT` 指令，与 §5.4.4 描述的自动迁移策略不一致 | **保留/已在 v4 中修改**。§1.4 的 Dockerfile prod stage 已在 v4 输出中补充了 `COPY entrypoint.sh .`、`RUN chmod +x entrypoint.sh` 和 `ENTRYPOINT ["./entrypoint.sh"]` 指令，与 §5.4.4 描述的 entrypoint 方案一致。容器启动时自动执行 `alembic upgrade head` 后启动 API 服务。 |
| `alembic.ini`（§5.4.2）中 `sqlalchemy.url` 硬编码为 `localhost:5432`，entrypoint.sh 在 Docker 容器内执行迁移时数据库服务主机名为 `db` | **保留/已在 v4 中修改**。§5.4.2 的 `alembic.ini` 已移除硬编码的 `sqlalchemy.url` 值，改为由 §5.4.5 的 `alembic/env.py` 从 `DATABASE_URL` 环境变量动态读取。该环境变量由 `docker-compose.yml` 中 `api` 服务的 `env_file: .env.prod` 注入，值为 `postgresql+psycopg2://farmeye:farmeye_pwd@db:5432/farmeye_db`，确保 Docker 容器内正确连接数据库。 |
| KingbaseES Docker 镜像 `kingbase/kb_v8:V008R006C008B0020`（§2.1.1 方案 A）的公开可获取性未经验证，需要商业授权 | **保留/已在 v4 中修改**。§2.1.1 已将方案 B（PostgreSQL 16 Alpine）升级为主要推荐方案，提供完整的 PostgreSQL 配置细节（连接串、`pg_isready` 健康检查、对应参数调优等）。KingbaseES 方案 A 降级为"可选"，标注需要商业授权，并提供了获取途径和手动镜像加载步骤。整个方案已以 PostgreSQL 16 为主线。 |

## 修订说明（v5）

| 审查意见 | 处理方式 |
|---------|---------|
| 依赖文件 `requirements.txt` 和 `requirements-dev.txt` 使用 `==X.Y.x` 格式，该格式不是有效的 pip 版本说明符，pip install 会报错 | **修改**。将所有 `==X.Y.x` 替换为 pip 兼容发布说明符 `~=X.Y.0`（如 `fastapi~=0.115.0`），该格式是有效的 pip 版本约束。同时在每个依赖文件末尾添加了版本约束说明，提示读者这些是示例版本约束，可根据实际发布情况调整。 |
| 数据库方案以 PostgreSQL 16 为主要推荐，KingbaseES 降级为可选方案，未充分响应架构文档对 KingbaseES 的明确要求 | **修改**。新增 §2.5"KingbaseES V8 完整等效配置"章节，提供完整的 KingbaseES 配置指引：Docker 镜像获取与加载步骤、完整 docker-compose 服务定义、连接字符串、1GB RAM 资源调优参数、备份脚本适配命令对照表。同时新增 §2.5.6"数据库选型决策说明"对比表，明确标注 PostgreSQL 适用于开发/测试阶段，KingbaseES 适用于生产部署/国产化合规场景，并说明两种数据库共享同一套 DDL 脚本的零迁移成本优势。更新目录以反映新增章节。 |
| Docker 测试用例 #4（§4.4.2）使用 `ksql -c 'SELECT 1'` 但主力方案是 PostgreSQL（该镜像中不存在 `ksql`） | **修改**。将测试用例 #4 的预期验证命令从 `ksql -c 'SELECT 1'` 替换为 `pg_isready -U farmeye -d farmeye_db`，与主力数据库方案（PostgreSQL 16）一致。 |
| 资源限制表（§3.5）声明 DB 内存限制为 256M，但 `docker-compose.prod.yml`（§1.5.2）将 DB 限制覆写为 384M，生产部署实际使用 384M | **修改**。§3.5 资源限制表中 DB 内存限制从 256M 更新为 384M，并补充说明：主 `docker-compose.yml` 使用 256M（开发环境默认值），生产覆写文件 `docker-compose.prod.yml` 提升至 384M，部署生产环境时必须合并启动。同步更新 §2.1.2 的内存分配计算表（DB 从 256MB→384MB，OS 从 424MB→296MB），增加说明阐明开发/生产两套数值的设计意图。 |
...
| 审查意见 | 处理方式 |
|---------|---------|
| Python 3.11 在 Ubuntu 25.04 上不可用 — §1.1 声称"Python 3.11+（与 Ubuntu 25.04 默认 Python 版本一致）"，§1.4 Dockerfile 通过 apt-get 安装 `python3.11` 和 `python3.11-venv`。Ubuntu 25.04 默认搭载 Python 3.13，其 apt 仓库不提供 python3.11 软件包 | **修改**。§1.1.1 将 Python 版本描述更新为"Python 3.13+（Ubuntu 25.04 默认搭载 Python 3.13）"。§1.4 Dockerfile 中 `apt-get install` 的 `python3.11` 和 `python3.11-venv` 替换为 `python3` 和 `python3-venv`（自动使用系统默认 Python 版本）。§1.4 多阶段构建说明同步更新为"安装 Python 3.13（Ubuntu 25.04 默认版本）"。 |
| Docker Compose profile 机制导致 api 与 api-dev 端口冲突 — 无 profile 的 `api` 服务始终启动，与 `api-dev` 在 `--profile dev` 下同时启动并争夺 8000 端口 | **修改**。采用审查意见中的修复路径 (a)：为 `api` 服务添加 `profiles: ["production"]` 属性，实现 api（production profile）与 api-dev（dev profile）通过 profile 机制互斥选择。同时，在所有涉及生产部署的 docker compose 命令中添加 `--profile production` 参数，涉及：§3.2.2 部署命令、§3.2.3 版本更新命令与回滚备注、§3.6.1 start.sh、§3.6.2 stop.sh（三个子命令）、§3.6.3 restart.sh、§5.1.3 VPS 部署循环、§6.2 快速部署命令。§1.5.1 中 api-dev 的注释同步更新以反映 profile 机制的设计意图。 |
...
| 审查意见 | 处理方式 |
|---------|---------|
| Dockerfile 缺少 .dockerignore 导致凭据泄露风险（§1.4 / §5.3.4 / §6.1） | **修改**。新增 §1.4.1 .dockerignore 配置章节，包含完整的排除规则；在 §6.1 文件清单中新增 `.dockerignore` 条目；在 §1.4 末尾增加安全说明。 |
| psycopg2-binary 不推荐用于生产环境（§1.2.1 / §1.4） | **修改**。将 `requirements.txt` 中的 `psycopg2-binary` 替换为 `psycopg2`；在 Dockerfile base 阶段新增 `build-essential`、`python3-dev`、`libpq-dev` 三项编译依赖并添加注释说明；增加镜像体积影响注释（约 150-200 MB）。 |
| 两套 Schema 初始化机制缺乏调和策略（§2.2.1 / §5.4 / entrypoint.sh） | **修改**。新增 §5.4.6 初始基准迁移章节，明确 init SQL 与 Alembic 的职责边界，选择方案 A（alembic stamp head）作为推荐调和策略；新增 §5.4.7 首次运行边界处理章节；更新 §5.4.4 entrypoint.sh 使其优雅处理首次部署时的边界情况。 |
| cleanup_expired_data() 异步定义但内部使用同步 SQLAlchemy 调用（§2.4） | **修改**。在函数文档字符串中添加设计决策说明，标注了两种可行的修复路径：通过 asyncio.to_thread() 包装或在 APScheduler 中切换为 ThreadPoolExecutor。 |
| 测试用例 #40 预期结果矛盾（200 与 503 并存）（§4.2.6） | **修改**。将预期结果从 `"200，status=degraded, HTTP 503"` 修正为 `HTTP 503，status=degraded`，与架构文档 §4.10.1 规定一致。 |
| 端口映射与 Nginx 配置不匹配（443 映射但无 SSL 监听）（§1.5.2 / §3.3.1 / §3.1.1） | **修改**。采用路径 A：在 §3.3.1 Nginx 配置中补充 SSL 监听（listen 443 ssl http2）、证书路径配置和 SSL 协议加密套件；新增 §3.3.3 证书管理章节，提供 Certbot + Let's Encrypt 自动化申请和续期步骤；更新 docker-compose.prod.yml 中 Nginx 的 SSL 证书 volume 挂载。 |
| docker compose --compatibility 依赖风险未文档化（§3.2.2 / §3.5 / §5.1.3） | **修改**。在 §3.5 补充了 `--compatibility` 模式版本间行为差异的说明，新增资源限制验证命令（docker inspect + jq），可选提及 mem_limit/mem_reservation 替代方案；在 §3.2.2 验证部署步骤中新增资源限制验证。 |
| Docker 守护进程内存预算未优化（§2.1.2） | **修改**。在内存分配计算表"OS + 缓存"行备注中补充"含 Docker 守护进程约 50-100 MB"。 |
| Uvicorn 参数硬编码，HOST/PORT/WORKERS 环境变量未被消费（§1.3.2 / §1.4） | **修改**。从 `.env.prod` 中移除 HOST/PORT/WORKERS 激活配置，改为注释状态并标注为预留字段。同步更新 §5.3.3 的 `.env.prod.example`。 |
| 测试数据库连接串硬编码（§4.3.1） | **修改**。将 `TEST_DATABASE_URL` 的值改为从 `TEST_DATABASE_URL` 环境变量读取，并设置默认值。 |
| 备份脚本缺少容器运行状态前提检查（§2.3.2 backup.sh） | **修改**。在 `docker exec` 前增加 `docker ps --format '{{.Names}}'` 容器状态检查逻辑，容器未运行时输出错误信息并中止备份。 |

## 修订说明（v8 — 迭代 v2 第 2 轮）

| 审查意见 | 处理方式 |
|---------|---------|
| cleanup_expired_data() 异步定义但内部使用同步调用，仅添加了设计决策注释（docstring）但未实际修改代码示例（§2.4） | **修改**。采纳路径 A：将函数从 `async def` 改为普通同步 `def`，更新 docstring 说明选择同步路径的理由（纯 IO 密集、错误处理更直观、在 ThreadPoolExecutor 中运行不影响事件循环）。函数内部代码（SessionLocal、db.execute、db.commit 等）保持不变，因其本身就是同步调用。 |
| §3.3.3 Certbot 续期 hook 脚本中容器名 `farmeye-ginx` 拼写错误（应为 `farmeye-nginx`）（§3.3.3） | **修改**。将 `docker exec farmeye-ginx nginx -s reload` 修正为 `docker exec farmeye-nginx nginx -s reload`，与 docker-compose.prod.yml 第 495 行定义的容器名一致。 |

## 修订说明（v9 — 迭代 v3）

| 审查意见 | 处理方式 |
|---------|---------|
| H1：entrypoint.sh 迁移失败后静默继续，不区分首次部署与真实迁移错误（§5.4.4） | **修改**。采用路径 A（推荐）：在 entrypoint.sh 中先执行 `alembic current` 检测 `alembic_version` 表是否存在版本记录。如果有版本记录（`STRICT_MIGRATION=true`），`alembic upgrade head` 失败时阻塞容器启动（`exit 1`）并输出详细错误信息（列出常见原因：迁移脚本 SQL 语法错误、数据库连接中断、版本历史冲突）。仅当无版本记录（首次部署）时，警告后继续。同步更新 §5.4.7 的描述以反映两阶段判断逻辑。 |
| M1：`datetime.utcnow()` 在 Python 3.13 中已弃用（§2.4） | **修改**。将 `from datetime import datetime, timedelta` 改为 `from datetime import datetime, timedelta, timezone`；将 `now = datetime.utcnow()` 替换为 `now = datetime.now(timezone.utc)`。由于数据库字段使用 `DEFAULT CURRENT_TIMESTAMP`（返回时区 naive 的 timestamp），在将 `cutoff_sensor` 和 `cutoff_control` 传入 DB 查询前通过 `.replace(tzinfo=None)` 去除时区信息，确保与时区 naive 的数据库列比较一致。 |
| M2：生产环境 API 端口暴露绕过 Nginx 安全层（§1.5.1） | **修改**。将 docker-compose.yml 中 `api` 服务的端口映射从 `"8000:8000"` 改为 `"127.0.0.1:8000:8000"`，监听 localhost 仅。Nginx 通过 Docker 内部网络（`farmeye-net`）以 `http://api:8000` 访问。`api-dev` 服务（开发模式，无 Nginx）保留原 `"8000:8000"` 不变。 |
| M3：文档三处对端口 8000 的处理存在目标冲突（§3.1.1 / §1.5.1 / §3.3） | **修改**。UFW 规则中 `sudo ufw allow 8000/tcp` 注释掉（改为 `# sudo ufw allow 8000/tcp`），并注明仅在无 Nginx 时开放。结合 M2 的端口映射修改（`127.0.0.1:8000:8000`），三处配置在逻辑上协调一致：UFW 不开放 8000，端口仅监听 localhost，Nginx 为唯一外部入口。 |
| M4：重复的"## 6. 附录"二级标题（§6） | **修改**。删除其中一个重复的 `## 6. 附录` 标题。 |
| M5：本地开发路径缺少 psycopg2 编译依赖说明（§1.1.1） | **修改**。在 `pip install` 步骤后增加 blockquote 说明：`psycopg2~=2.9.0` 需要宿主机安装 `libpq-dev`、`build-essential`、`python3-dev`；对 Windows/macOS 开发者提示可临时使用 `psycopg2-binary` 替代（仅限本地开发）；说明生产环境在 Docker base 阶段预装编译依赖。 |
