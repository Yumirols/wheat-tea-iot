...
> **文档版本**: v8（迭代 v2 修订）
> **修订依据**: 迭代 v2 要求（a_v2_iteration_requirement.md）
...
sudo ufw allow 8000/tcp         # API（如无 Nginx 时）
...
# 输出当前 Alembic 版本号（首次运行时为 None，不阻塞启动）
alembic current 2>&1 || true

# 尝试执行迁移；首次部署时 init SQL 已完成基线初始化，迁移可能未 stamp
...
entrypoint.sh 已在 §5.4.4 中定义为最终的改进版本。其首次运行边界处理逻辑如下：

1. 输出当前 Alembic 版本号（首次运行时为 `None`），便于运维排查
2. 尝试执行 `alembic upgrade head`；如果因首次部署未 stamp 而失败，**不退出容器启动流程**，而是输出警告信息，提示部署后需执行 `alembic stamp head`
3. 无论迁移是否成功，均继续启动 API 服务

> **说明**：首次部署时，init SQL 已通过 PostgreSQL 的 `/docker-entrypoint-initdb.d/` 机制完成了建表，数据库 Schema 处于最新状态但 Alembic 无版本记录。此时 `alembic upgrade head` 可能报错（若无初始迁移脚本）。改进后的 entrypoint.sh 会捕获此错误、输出警告并继续启动 API 服务，**不阻塞容器启动流程**。在生产运维中，应在部署完成后参考 §5.4.6 的调和策略执行 `alembic stamp head` 以完成版本对齐。
