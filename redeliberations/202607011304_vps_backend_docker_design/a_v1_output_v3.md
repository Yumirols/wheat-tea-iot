...
1. [Python API 后端 (FastAPI) 容器化](#1-python-api-后端-fastapi-容器化)
   ...
   - 1.6 生产/开发配置分离策略
   - 1.7 健康检查配置
2. [金仓数据库 (KingbaseES)](#2-金仓数据库-kingbasees)
   - 2.1 镜像适配方案
   - 2.2 数据库初始化脚本
   - 2.3 数据持久化与备份策略
   - 2.4 数据保留与清理策略
...
# 生产模式：多 worker 由 Gunicorn 管理（单 worker 直接 uvicorn）
...
> - `prod` 阶段：在 base 基础上仅复制应用代码，不包含任何开发工具和编译工具链，镜像体积最小化。
...
services:
  ...
  db:
    ...
    healthcheck:
      test: ["CMD-SHELL", "ksql -U ${DB_USER} -d ${DB_NAME} -c 'SELECT 1' 2>/dev/null || nc -z localhost 5432"]
      interval: 10s
      timeout: 5s
      retries: 5
      start_period: 60s
...
2. **数据库服务**（docker-compose.yml 中定义）：
   ...
   - 检查命令：`ksql -U ${DB_USER} -d ${DB_NAME} -c 'SELECT 1' 2>/dev/null || nc -z localhost 5432`
...
KingbaseES V8 提供官方 Docker 镜像 `kingbase/kb_v8:V008R006C008B0020`。由于 KingbaseES 兼容 PostgreSQL 协议，可选的适配方案有三种：

| 方案 | 描述 | 优点 | 缺点 | 推荐 |
|------|------|------|------|:----:|
| **A** | 使用 KingbaseES 官方 Docker 镜像 | 原生国产化，完全兼容目标环境 | 镜像较大（~1.5GB），内存占用较高 | **推荐** |
| **B** | 使用 PostgreSQL 16 镜像 + KingbaseES 兼容层 | 镜像小（~400MB），社区支持好 | 非纯国产方案，部分 KingbaseES 特有语法不兼容 | 备选 |
| **C** | 宿主机直接安装 KingbaseES | 无容器化开销 | 污染宿主机环境，与容器网络隔离方案冲突 | 不推荐 |

**选用方案 A**，理由：
- VPS 部署的核心目标之一就是使用 KingbaseES 国产数据库
- 1GB RAM 通过参数调优可满足 KingbaseES 运行需求
- 与后续国产化认证需求一致

#### 2.1.2 1GB RAM 环境下的 KingbaseES 配置调优

针对 VPS 1GB RAM 约束，KingbaseES 关键参数调整如下：

```ini
# postgresql.conf 覆盖参数
shared_buffers = 128MB          # 默认通常为 256MB，降至 128MB
effective_cache_size = 256MB    # 操作系统缓存预估
work_mem = 4MB                  # 每排序操作内存，降低以支持并发
maintenance_work_mem = 32MB     # 维护操作（VACUUM、CREATE INDEX）
...
| 组件 | 分配内存 | 说明 |
|------|---------|------|
| KingbaseES | 384MB | shared_buffers 128MB + 进程 + WAL + 缓存 |
...
volumes:
  db_data:          # KingbaseES 数据文件，位于 /var/lib/docker/volumes/
...
| 数据库（KingbaseES） | Docker json-file driver | `docker logs farmeye-db` | max-size=10m, max-file=3 |
...
> **说明**：`deploy.resources.limits` 在 docker compose 的 `--compatibility` 模式下生效，会转换为容器的 `--memory` 和 `--memory-reservation` 参数。不加 `--compatibility` 标志时仅 swarm 模式下生效，但显式写在 compose 中作为文档化设计，配合 `--compatibility` 标志确保限制生效。
...
| 4 | DB 健康检查通过 | 等待 60s 后 | `ksql -c 'SELECT 1'` 执行成功 | 健康检查 |
...
| 方向 | 建议 | 预期效果 |
|------|------|---------|
| 数据库连接池 | SQLAlchemy `pool_size=2, max_overflow=2` | 控制最大连接数为 4，避免撑爆 KingbaseES |
