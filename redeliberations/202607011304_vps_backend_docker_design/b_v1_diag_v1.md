...
- **位置**: §1.2.1 `server/requirements.txt`，第 102 行
...
- **改进建议**:
  ...
  2. 在 Dockerfile `base` 阶段的 `apt-get install` 指令中添加 `libpq-dev` 编译依赖：
     ```
     RUN apt-get update && apt-get install -y --no-install-recommends \
         python3 \
         python3-venv \
         python3-pip \
         libpq-dev \      # <-- 新增：psycopg2 编译依赖
         curl \
         ca-certificates \
         && rm -rf /var/lib/apt/lists/*
     ```
  3. `requirements-dev.txt` 中的 `httpx` 版本冗余说明（"与 requirements.txt 中的 httpx 版本应保持一致"）已在 v5 保持，但上一条修改不影响此条。
...
DIAG_WRITTEN:E:\dev\wheat-tea-iot\redeliberations\202607011304_vps_backend_docker_design\b_v1_diag_v1.md
