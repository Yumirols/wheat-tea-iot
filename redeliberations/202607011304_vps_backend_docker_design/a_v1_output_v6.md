...
| OS + 缓存 | 296MB | Ubuntu 25.04 内核 + 文件系统缓存 |
...
| 40 | 健康检查（数据库断开） | DB 连接失败 | 200，`status=degraded`, HTTP 503 | Unit |
...
# 使用独立的测试数据库连接字符串
TEST_DATABASE_URL = "postgresql+psycopg2://farmeye:farmeye_pwd@localhost:5432/farmeye_test"
```
