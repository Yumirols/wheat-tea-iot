...
项目使用 Python 3.11+（与 Ubuntu 25.04 默认 Python 版本一致），推荐使用 `venv` 管理虚拟环境：
...
> **多阶段构建说明**：
> - `base` 阶段：安装 Python 3.11、创建虚拟环境、安装生产依赖。此阶段为 prod 和 dev 的共享基础。
...
# 回滚（如需）
# docker compose up -d --no-deps api  # 使用旧镜像
```
