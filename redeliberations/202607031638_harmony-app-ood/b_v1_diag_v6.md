# 农眼卫士 FarmEye Guard v1.0 — OOD 设计方案可落地性审查报告（v6，响应质询版）
...
| 审查方法 | 基于 v5 审查报告及质询文件逐项复核，按质询意见修订建议 |
| 本版变更 | 响应质询 CH1–CH3：CH1 接受→修订 H10 补充 PollingManager 轮询交互分析；CH2 接受→新增 H11 现有文件兼容性评估；CH3 接受→修订 H5 补充初次加载错误处理路径 |
...
**v6 审查：10 项高严重度问题、4 项中严重度问题、3 项低严重度问题**（依据质询 CH1–CH3 修订），详情如下。
...
### H5. 所有页面 `aboutToAppear` 中编排异步加载 + `pushUrl` 场景下轮询停止契约不可执行（修订）
...
**改进建议**：
- 每个页面增加 `@State private isLoading: boolean = true` 和 `@State private errorMessage: string | null = null`
- `aboutToAppear()` 中通过非 async 方式触发 `loadData()` 异步方法，**必须包含完整的错误处理契约**：
  ```typescript
  aboutToAppear() {
    this.isLoading = true
    this.loadData()
  ...
  ```
- `loadData()` 统一返回 `Promise<void>`，**同步段异常**（第一个 `await` 前抛出的参数校验等错误）由函数体内部的 `try-catch` 捕获并转为 `Promise.reject`，确保不会传播到 `aboutToAppear` 调用方导致白屏
- `build()` 中根据 `isLoading` 和 `errorMessage` 条件渲染加载指示器、错误状态或数据内容；错误状态下提供"重新加载"按钮
- **场景 E 修正为**：
...
### H10. 农业 IoT 场景弱网韧性完全未覆盖（修订，补充 PollingManager 交互分析）
...
**4. 重试与轮询调度交互风险（新增）**
`RetryPolicy`（指数退避：baseDelayMs=1000, maxDelayMs=10000, maxRetries=3）与 `PollingManager` 的 10s `setInterval` 存在竞争：
- 单个轮询 tick 最大耗时：初始请求 ~2s + 重试1(1s延迟+~2s) + 重试2(2s延迟+~2s) + 重试3(4s延迟+~2s) = 最长 ~15s
- `setInterval` **不等待回调完成**，第 10s 时下一个 tick 仍会触发，导致：
  - **并发请求**：前一个 tick 的重试与下一个 tick 的请求同时执行
  - **UI 状态竞争**：两个请求的响应到达时序不确定，`@State` 被交替更新 → UI 闪烁
  - **带宽浪费**：弱网场景下并发请求加重网络拥塞，与弱网韧性目标背道而驰
  - **CacheManager 缓存竞争**：前一个重试写入缓存后，后一个 tick 覆盖写回更旧的数据
...
- **轮询重试交互约束**：
  - `PollingManager` 必须采用以下策略之一治理重试与轮询的竞争：
    - **串行模式**：使用递归 `setTimeout` 替代 `setInterval`，上一个 tick（含重试）完全结束后再调度下一个 10s
    - **跳略模式**：`setInterval` + 运行标志位，若前一个 tick 未完成则跳过当前 tick，确保同一时间只有一个请求序列在执行
    - **Tick 超时截断**：`RetryPolicy` 中设置 `tickTimeoutMs: 8000`，确保重试在下一个 tick 前完成或放弃，避免累积效应
  - 轮询回调中的重试在写入 `CacheManager` 时需检查页面销毁标记，避免写入已卸载页面的 `@State`

---

### H11. 现有 `common/api.ets` 与两层 HTTP 架构的兼容性未评估（新增）

**位置**：`requirement.md` §5（第 83 行 `common/api.ets` 已有文件声明）、`a_v1_design_v3.md` §1 架构描述（第 9–13 行）

**问题描述**：
需求明确 `common/api.ets` 为**已有文件**（"已有文件，需在此基础上设计实现"）。v3 设计提出了"原始传输层（`api.ets`）→ 业务门面层（`HttpClient.ets`）"的两层 HTTP 封装结构，将现有 `api.ets` 重新定义为 `@ohos.net.http` 的轻量适配器。但审查未评估该重新定义的三项前提风险：

1. **职责重叠风险**：现有 `api.ets` 的描述为"HTTP 请求封装"，实际实现可能已包含认证头注入（`X-API-Key`）、JSON 解析或基础路径拼接等逻辑。若如此，则这些职责与新建的 `HttpClient` 重叠——要么重构现有 `api.ets`，要么修改设计职责分配
2. **依赖方向合规性**：设计规定 `services/` → `common/`，`HttpClient` 位于 `services/`，需导入 `common/api.ets` 的模块级函数。如果现有 `api.ets` 以类或实例对象暴露（而非具名导出函数），需确认导入方式是否与 ArkTS 模块系统兼容
3. **迁移成本未评估**：即使现有 `api.ets` 需要大规模重构以适应纯传输层角色（移除认证、JSON 解析等职责），该重构成本也未在设计或审查中体现。设计方案的部分可行性建立在对现有 `api.ets` 角色的假设之上，但该假设未被验证或标记为风险

**严重程度**：高 — 若现有 `api.ets` 的实现与设计假设不一致，整个 HTTP 两层架构需要调整

**改进建议**：
- **首要行动**：审查现有 `common/api.ets` 的实际实现内容，逐项对比设计分配给 `api.ets` 和 `HttpClient.ets` 的职责边界（认证注入、路径拼接、JSON 解析、错误码映射），标记重叠或冲突的部分
- **评估现有代码的迁移路径**：决定完全重写、增量剥离（将业务语义逐步迁移到 `HttpClient`）或按原样保留（若现有 `api.ets` 已是纯传输层，则设计假设成立无需改动）
- **在审查报告中补充"现有文件兼容性评估"**：将 `api.ets` 的实际实现与设计角色的差距作为已知风险条目登记，明确编码前的必要前置动作
...
| H10 | 架构遗漏 | 修订 | IoT 弱网场景的缓存、重试、离线 UI 未覆盖；重试与轮询交互需串行/跳略模式治理 |
| H11 | 覆盖遗漏 | 新增 | 现有 `common/api.ets` 与两层 HTTP 架构的兼容性未评估，设计假设未验证 |

**关键依赖链**：`H1(image_path 语义)` → 影响 `H4(requestRaw 紧急性)` → 影响 API 覆盖完整性检查中 `GET /image/{image_id}` 的行；H9、H10、H11 为独立新增。H11(现有 api.ets 实现检查) 为所有 HTTP 架构相关判断的前置条件。

**建议**：上述 10 项高严重度问题全部修复，且 `image_path` 语义经 API 确认、现有 `api.ets` 实现经审查后，方可进入编码阶段。

---

## 修订说明（v6 审查报告修订，响应质询 v3）

### 针对质询 CH1 的响应

| 质询内容 | 判定 | 处理 |
|---------|------|------|
| H10 RetryPolicy 与 PollingManager 10s 间隔的交互未定义，setInterval 与指数退避存在竞争风险（并发请求、UI 状态竞争、带宽浪费、缓存竞争） | **接受** | 在 H10 改进建议中新增"轮询重试交互约束"段落，定义串行模式（setTimeout）、跳略模式（运行标志位）、Tick 超时截断三种解决方案；补充说明轮询回调重试写入缓存时需检查页面销毁标记 |

### 针对质询 CH2 的响应

| 质询内容 | 判定 | 处理 |
|---------|------|------|
| 现有 `common/api.ets` 与两层架构的兼容性未评估，设计依赖于未经验证的前提假设（职责重叠风险、依赖方向合规性、迁移成本） | **接受** | 新增 H11 高严重度问题，覆盖职责重叠风险、依赖方向合规性、迁移成本三项子维度；建议审查现有实现后调整设计职责分配 |

### 针对质询 CH3 的响应

| 质询内容 | 判定 | 处理 |
|---------|------|------|
| H5 aboutToAppear 初始加载的错误处理未定义（同步段异常导致白屏 + 异步 Promise rejection 未处理） | **接受** | 修订 H5 改进建议，补充完整的 `loadData()` 错误处理契约（`.catch()` 捕获 + `@State errorMessage` + 重新加载触发器 + 同步段异常内部 try-catch 确保不传播到调用方） |

---

```
DIAG_WRITTEN:E:\dev\wheat-tea-iot\redeliberations\202607031638_harmony-app-ood\b_v1_diag_v6.md
