# 农眼卫士 FarmEye Guard v1.0 — 鸿蒙移动应用 OOD 设计方案（v5）
...
**职责**：
- 声明 `@State` 装饰的本地状态（UI 渲染数据）：每个页面增加 `@State private isLoading: boolean`、`@State private errorMessage: string | null`、`@State private connectivityStatus: 'online' | 'offline'`（初始值为 `'online'`，`aboutToAppear` 中短暂设为 `'loading'`），以及 `@State private alarmMessage: string` 和 `@State private alarmSeverity: 'mild' | 'moderate' | 'severe'`（用于 AlertBanner 展示）
...
```
DESIGN_WRITTEN:E:\dev\wheat-tea-iot\redeliberations\202607031638_harmony-app-ood\a_v3_design_v1.md
