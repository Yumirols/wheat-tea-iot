...
static uint32_t g_wifi_ip = 0;

int8_t wifi_get_rssi(void) { return g_wifi_rssi; }
uint32_t wifi_get_ip(void) { return g_wifi_ip; }
struct netif *wifi_get_netif(void) { return g_wifi_netif; }
...
errcode_t wifi_connect(void)
{
    ...
    }
    {
        ...
        do {
            if (++scan_retry > 5) {
                printf("WiFi scan exhausted, AP not found!\r\n");
                return ERRCODE_FAIL;
            }
            printf("Start Scan (%d/5)\r\n", scan_retry);
            ...
            if (example_get_match_network(expected_ssid, key, &expected_bss) != ERRCODE_SUCC) {
                printf("AP [%s] not found, retry...\r\n", expected_ssid);
                continue;
            }

            printf("AP found, connecting...\r\n");
            /* 启动连接 */
    ...
    for (uint8_t i = 0; i < WIFI_STA_IP_MAX_GET_TIMES; i++) {
        ...
        if (netif_p->ip_addr.u_addr.ip4.addr != 0) {
            ...
            g_wifi_netif = netif_p;
            g_wifi_ip = netif_p->ip_addr.u_addr.ip4.addr;
            /* 连接成功 */
            printf("STA connect success.\r\n");
            mqtt_app_start();//开始启动MQTT
            return ERRCODE_SUCC;
        }
    }
